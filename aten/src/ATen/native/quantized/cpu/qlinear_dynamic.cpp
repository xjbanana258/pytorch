...
at::Tensor& PackedLinearWeightFp16::apply_dynamic_impl(
  ...
  // Add bias term: fused row-wise add avoids the TensorIterator dispatch of add_.
  if (bias_.has_value()) {
    TORCH_CHECK(bias_->dim() == 1);
    const at::Tensor bias_contig = bias_->contiguous();
    TORCH_CHECK(bias_contig.numel() == N);
    const float* const __restrict bias_ptr = bias_contig.const_data_ptr<float>();
    const int64_t grain =
        (N > 0) ? std::max<int64_t>(1, at::internal::GRAIN_SIZE / N) : 1;
